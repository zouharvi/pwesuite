...
def train_step(vae_model, train_loader, optimizer, loss_multipliers, limit_iter_per_epoch=None):
    ...
    for i, data in enumerate(train_loader):
        ...
        loss = (
            kl_loss * loss_multipliers['kl'] +
            recon_loss * loss_multipliers['recon']
        )
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def train(args, vocab, vae_model, loss_multipliers):
    ...
    train_dset = IPATokenDataset(
        [f'data/ipa_tokens_{lang}.txt' for lang in args.lang_codes], vocab, split_bounds=(0, args.train_ratio))
    train_loader = DataLoader(train_dset, shuffle=True, **loader_kwargs)
    val_dset = IPATokenDataset(
        [f'data/ipa_tokens_{lang}.txt' for lang in args.lang_codes], vocab, split_bounds=(args.train_ratio, 1.0))
    ...
    val_loss_dict = validate_step(
        vae_model, val_loader, loss_multipliers, evaluator)
    ...
    for ep in range(args.epochs):
        ...
        if spearman > best_spearman:
            ...
            save_model(
                vae_model, optimizer, args,
                ipa_vocab, ep, model_save_path
            )

        print(
            f'< epoch {ep} >  (elapsed: {time.time() - t:.2f}s, decode time: {time.time() - train_time:.2f}s)')
